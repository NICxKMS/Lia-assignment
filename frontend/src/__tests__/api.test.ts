...
import type { ConversationDetail, ConversationSummary } from "../lib/api";
...
describe("API Module", () => {
			...
			it("fetches conversation with pagination params", async () => {
				...
				expect(mockFetch).toHaveBeenCalledWith(
					expect.stringMatching(
						/\/chat\/conversation\/conv-123\?limit=20&offset=40/,
					),
					expect.anything(),
				);
			});
	...
	describe("Error Handling", () => {
		...
		it("handles API errors by throwing response", async () => {
			...
			await expect(chatApi.getHistory()).rejects.toMatchObject({
				ok: false,
				status: 400,
			});
		});
