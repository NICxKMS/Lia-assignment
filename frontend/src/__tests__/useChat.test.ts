	...
	describe("sendMessage", () => {
		it("sends message and receives streamed response", async () => {
			const events = [
				{
					event: "start",
					data: { conversation_id: "conv-123", message_id: "msg-123" },
				},
		...
		it("updates sentiment on user message", async () => {
			...
			expect(userMessage?.sentiment).toEqual({
				score: 0.9,
				label: "Positive",
				emotion: "happy",
			});
			expect(userMessage?.cumulativeSentiment).toEqual({
				score: 0.7,
				label: "Positive",
			});
		});

		it("sends with credentials include for cookie auth", async () => {
			mockFetch.mockResolvedValue(
				createSSEResponse([{ event: "done", data: {} }]),
			);
