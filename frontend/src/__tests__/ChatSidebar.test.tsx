			...
			(
				...
				const filtered = Object.fromEntries(
					Object.entries(props).filter(
						([key]) =>
							!motionProps.includes(key as (typeof motionProps)[number]),
					),
...
describe("ChatSidebar", () => {
	...
	describe("Conversation List", () => {
		...
		it("renders conversation titles", () => {
			const history = [createMockConversation("1", "My First Chat", 0)];
		...
		it("truncates long conversation titles", () => {
			...
			const history = [createMockConversation("1", longTitle, 0)];
		...
		it('shows "New conversation" for null titles', () => {
			const history = [createMockConversation("1", null, 0)];
		...
		it("highlights current conversation", () => {
			...
			expect(chat1Elements[0].closest('[role="button"]')).toHaveClass(
				"text-primary",
			);
		});
	});
	...
	describe("Delete All", () => {
		it("renders delete all button when history has items", () => {
			...
			const deleteAllButtons = screen.getAllByRole("button", {
				name: /delete all/i,
			});
