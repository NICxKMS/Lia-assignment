...
describe('ChatInspector', () => {
  const defaultProps = {
    ...
    chartData: [] as Array<{ id: string; name: number; score: number; label?: string; content?: string }>,
    isOpen: true,
    onClose: vi.fn(),
    method: 'llm_separate',
    setMethod: vi.fn(),
  ...
  describe('Overall Sentiment', () => {
    ...
    it('displays overall sentiment from last message with cumulative sentiment', () => {
      ...
      render(<ChatInspector {...defaultProps} messages={messagesWithSentiment} />)
  ...
  describe('Session Trend Chart', () => {
    ...
    it('renders chart when chart data is available', () => {
      const chartData = [
        { id: 'msg1', name: 1, score: 0.5, label: 'Positive', content: 'Hello' },
        { id: 'msg2', name: 2, score: -0.3, label: 'Negative', content: 'Goodbye' },
      ]
    ...
    it('shows emotion when available in cumulative sentiment', () => {
      const messagesWithEmotion: ChatMessage[] = [
        createMockMessage({
          ...
          role: 'assistant',
          content: 'Response',
          cumulativeSentiment: { score: 0.8, label: 'Positive', emotion: 'joy' },
        }),
      ]
      
      render(<ChatInspector {...defaultProps} messages={messagesWithEmotion} />)
