...
import { Activity, Hash, X, Info, Zap, MessageSquare } from 'lucide-react'
...
const SENTIMENT_COLORS = {
  Positive: { bg: 'bg-emerald-500/10', text: 'text-emerald-400', border: 'border-emerald-500/20', gradient: 'from-emerald-500 to-teal-400' },
  Negative: { bg: 'bg-rose-500/10', text: 'text-rose-400', border: 'border-rose-500/20', gradient: 'from-rose-500 to-orange-400' },
  Neutral: { bg: 'bg-blue-500/10', text: 'text-blue-400', border: 'border-blue-500/20', gradient: 'from-blue-500 to-indigo-400' },
}
...
const SentimentCard = memo<{
  ...
  const colors = SENTIMENT_COLORS[sentiment?.label as keyof typeof SENTIMENT_COLORS] || SENTIMENT_COLORS.Neutral
      ...
      <CardContent className={cn("px-4 pb-4", compact && "pt-0")}>
        {sentiment ? (
          <div className="relative">
            {!compact && (
              <div className={cn('absolute -top-2 left-0 w-full h-1 rounded-full opacity-70 bg-gradient-to-r', colors.gradient)} />
            )}
            
            <div className="flex items-end justify-between mb-2 pt-2">
              <span className={cn("font-bold font-mono tracking-tighter", compact ? "text-2xl" : "text-4xl")}>
                {(sentiment.score * 100).toFixed(0)}
              </span>
              <Badge variant="outline" className={cn('text-[10px] font-bold uppercase', colors.bg, colors.text, colors.border)}>
                {sentiment.label}
              </Badge>
            </div>
                    ...
                    <AreaChart data={chartData}>
                          ...
                          if (active && payload && payload.length) {
                            ...
                            return (
                              <div className="bg-popover border border-border p-2 rounded-lg shadow-lg max-w-[200px]">
                                <p className="text-xs font-bold mb-1">Message {data.name}</p>
                                <div className="flex items-center justify-between gap-4 mb-1">
                                  <span className="text-xs text-muted-foreground">{data.label}</span>
                                  <span className="text-xs font-mono font-bold">{(data.score * 100).toFixed(0)}%</span>
                                </div>
                                {data.content && (
                      ...
                      <Area 
                        ...
                        dot={(props) => {
                          ...
                          const isFocused = userMessages[focusedUserMessageIndex]?.id === payload.id;
            ...
            <div className="space-y-2 max-h-[400px] overflow-y-auto pr-2 scrollbar-thin scrollbar-thumb-border scrollbar-track-transparent">
              {userMessages.length > 0 ? (
                userMessages.map((msg) => {
                  ...
                  const colors = SENTIMENT_COLORS[sentiment?.label as keyof typeof SENTIMENT_COLORS] || SENTIMENT_COLORS.Neutral;
                  const isFocused = userMessages[focusedUserMessageIndex]?.id === msg.id;
                  ...
                  return (
                    <Card 
                      ...
                      className={cn(
                        ...
                        isFocused && "ring-2 ring-primary border-primary shadow-md scale-[1.02]"
                      )}
                    >
                      <div className={cn("h-1 w-full opacity-50", colors.bg)} />
                      <CardContent className="p-3 space-y-2">
                        <p className="text-xs text-muted-foreground line-clamp-3 italic" title={msg.content}>
                          "{msg.content}"
                        </p>
                        
                        {sentiment ? (
                          <div className="flex items-center justify-between">
                            <Badge variant="outline" className={cn('text-[10px] font-bold uppercase h-5', colors.bg, colors.text, colors.border)}>
                              {sentiment.label}
                            </Badge>
