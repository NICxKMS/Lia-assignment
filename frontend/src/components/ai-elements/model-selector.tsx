...
// Capability badges
const capabilityConfig: Record<string, { icon: React.ReactNode; label: string; color: string }> = {
  reasoning: { icon: <Brain className="w-2.5 h-2.5" />, label: 'Reasoning', color: 'text-purple-400 bg-purple-500/10' },
  vision: { icon: <span>👁</span>, label: 'Vision', color: 'text-blue-400 bg-blue-500/10' },
  code: { icon: <span>{'</>'}</span>, label: 'Code', color: 'text-green-400 bg-green-500/10' },
  fast: { icon: <Zap className="w-2.5 h-2.5" />, label: 'Fast', color: 'text-yellow-400 bg-yellow-500/10' },
}

// Marquee text component for long model names - memoized
const MarqueeText = memo<{ text: string; className?: string }>(({ text, className }) => {
        ...
        <div className="flex items-center gap-2 min-w-0 flex-1">
          {selectedModel && (
            <div className="w-5 h-5 rounded flex items-center justify-center bg-muted text-muted-foreground shrink-0">
              {selectedModel.icon || providerIcons[selectedModel.provider.toLowerCase()] || <Sparkles className="w-3 h-3" />}
            </div>
          )}
          <div className="min-w-0 flex-1 overflow-hidden">
            <MarqueeText text={selectedModel?.name || placeholder} className="text-sm" />
          </div>
        </div>
      </SelectTrigger>
              ...
              >
                <div className="flex items-center gap-2">
                  <div className="w-5 h-5 rounded flex items-center justify-center bg-muted text-muted-foreground text-xs">
                    {model.icon || providerIcons[model.provider.toLowerCase()] || <Sparkles className="w-3 h-3" />}
                  </div>
    ...
    <DropdownMenu open={open} onOpenChange={setOpen}>
      <DropdownMenuTrigger asChild>
        <Button
          ...
          role="combobox"
          aria-expanded={open}
          className={cn('h-9 justify-between gap-2 font-normal max-w-[200px]', className)}
        >
          <div className="flex items-center gap-2 min-w-0 flex-1">
            {selectedModel && (
            ...
            <div className="min-w-0 flex-1 overflow-hidden">
              <MarqueeText text={selectedModel?.name || "Select model"} className="text-sm" />
            </div>
          </div>
              ...
              {providerModels.map(model => (
                ...
                >
                  <div className={cn(
                    ...
                    value === model.id ? 'bg-primary text-primary-foreground' : 'bg-muted text-muted-foreground'
                  )}>
                    {model.icon || providerIcons[model.provider.toLowerCase()] || <Sparkles className="w-3 h-3" />}
                  </div>
        ...
        >
          {selectedModel && (
            <span className="w-4 h-4 rounded-full flex items-center justify-center bg-primary/10 text-primary">
              {selectedModel.icon || providerIcons[selectedModel.provider.toLowerCase()] || '✨'}
            </span>
          )}
        ...
        {models.map(model => (
          ...
          >
            <span className="w-4 h-4 rounded-full flex items-center justify-center bg-muted text-muted-foreground text-[10px]">
              {model.icon || providerIcons[model.provider.toLowerCase()] || '✨'}
            </span>
